...
public class Shell {
	...
	public String command(String command, int waitTimeSecond) {
		String result = "";
		try {
			bw.write(command);
			bw.newLine();
			...
			while (true) {
				if ((line = br.readLine()) == null) {
					if((System.currentTimeMillis() - startTime)>waitTimeSecond*1000){
						break;
					}
					break;
				} else {
//					System.out.println(line);
					...
					if (line.contains(command)&&line.contains("[")&&line.contains("]")&&line.contains("~")&&line.contains("#"))
						startflag = true;
